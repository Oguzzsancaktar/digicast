...
const findRegisterById = async (id, populate = '') => {
  const [customer] = await Register.aggregate([
    ...
    },
    {
      $lookup: {
        from: 'customers',
        localField: 'reliableRegisters.reliableId',
        foreignField: '_id',
        as: 'reliableRegisters.reliable'
      }
    },
    {
      $lookup: {
        from: 'relativetypes',
        localField: 'reliableRegisters.relativeType.relativeTypeId',
        foreignField: '_id',
        as: 'reliableRegisters.relativeType.relativeType'
      }
    },
    {
      $unwind: {
        path: '$reliableRegisters.reliable',
        preserveNullAndEmptyArrays: true
      }
    },
    {
      $unwind: {
        path: '$reliableRegisters.relativeType.relativeType',
        preserveNullAndEmptyArrays: true
      }
    },
    {
      $group: {
        ...
        reliableRegisters: { $push: '$reliableRegisters' }
      }
    }
  ]).exec()
  return customer
